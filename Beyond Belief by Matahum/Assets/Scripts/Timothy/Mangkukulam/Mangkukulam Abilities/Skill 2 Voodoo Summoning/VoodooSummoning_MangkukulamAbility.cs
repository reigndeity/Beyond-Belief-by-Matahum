    ...
    IEnumerator SummonVoodoo(GameObject user)
    {
        ...
        for (int i = 0; i < voodooCount; i++)
        {
            ...
            if (NavMesh.SamplePosition(basePos, out NavMeshHit hit, 5f, NavMesh.AllAreas))
            {
                ...
                GameObject summonCircle = Instantiate(summoningCirclePrefab, hit.position, summoningCirclePrefab.transform.rotation, user.transform.parent);
                Destroy(summonCircle, 5);
            }
        }
